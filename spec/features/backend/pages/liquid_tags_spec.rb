...
RSpec.describe 'Backend - Liquid Pages (HTML)', type: :feature do
  def fill_in_page_form_content_with(content)
    fill_in 'Title', with: 'Amazing'
    fill_in 'Slug', with: 'amazing'
    fill_in 'Content', with: content
  end

  def fill_in_and_submit_page_form_content_with(content)
    fill_in_page_form_content_with(content)
  ...
  describe 'creation with Liquid tags' do
    ...
    describe 'successful' do
      ...
      before { visit '/admin/pages/new' }

      it 'accepts valid `asset` tag' do
        fill_in_and_submit_page_form_content_with("{% asset 'amazing.jpg' %}")

        expect(page).to have_content('Page was successfully created.')
      end

      it 'accepts valid `collection` tag' do
        fill_in_and_submit_page_form_content_with(collection_content)

        expect(page).to have_content('Page was successfully created.')
      end

      it 'accepts valid `collectionfor` tag' do
        fill_in_and_submit_page_form_content_with(collectionfor_content)
      ...
      it 'accepts valid `gist` tag' do
        ...
        fill_in_and_submit_page_form_content_with(content)
      ...
      it 'accepts valid `widget` tag' do
        fill_in_and_submit_page_form_content_with("{% youtube 'amazing' %}")

        expect(page).to have_content('Page was successfully created.')
      end

      it 'accepts valid `youtube` tag' do
        fill_in_and_submit_page_form_content_with("{% youtube 'NOGEyBeoBGM' %}")
    ...
    describe 'unsuccessful' do
      ...
      %w[asset gist noembed vimeo widget youtube].each do |tag|
        it "fails without key for `#{tag}` tag" do
          fill_in_and_submit_page_form_content_with("{% #{tag} %}")
