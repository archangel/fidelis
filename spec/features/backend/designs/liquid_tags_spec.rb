...
RSpec.describe 'Backend - Liquid Designs (HTML)', type: :feature do
  def fill_in_design_form_content_with(content)
    fill_in 'Name', with: 'Amazing'
    fill_in 'Content', with: content
  end

  def fill_in_and_submit_design_form_content_with(content)
    fill_in_design_form_content_with(content)
  ...
  describe 'creation with Liquid tags' do
    ...
    describe 'successful' do
      ...
      before { visit '/admin/designs/new' }

      it 'accepts valid `asset` tag' do
        fill_in_and_submit_design_form_content_with("{% asset 'amazing.jpg' %}")

        expect(page).to have_content('Design was successfully created.')
      end

      it 'accepts valid `collection` tag' do
        fill_in_and_submit_design_form_content_with(collection_content)

        expect(page).to have_content('Design was successfully created.')
      end

      it 'accepts valid `collectionfor` tag' do
        fill_in_and_submit_design_form_content_with(collectionfor_content)
      ...
      it 'accepts valid `gist` tag' do
        ...
        fill_in_and_submit_design_form_content_with(content)
      ...
      it 'accepts valid `widget` tag' do
        fill_in_and_submit_design_form_content_with("{% youtube 'amazing' %}")
    ...
    describe 'unsuccessful' do
      ...
      %w[asset gist noembed vimeo widget youtube].each do |tag|
        it "fails without key for `#{tag}` tag" do
          fill_in_and_submit_design_form_content_with("{% #{tag} %}")
